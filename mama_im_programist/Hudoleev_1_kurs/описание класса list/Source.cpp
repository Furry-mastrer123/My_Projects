...
enum errortype { stackisfull, stackisempty };
...
public:
	void errorprint();
	stackexception(errortype exnum)
	{
		this->errornum = exnum;
	}
};

void stackexception::errorprint()
{
	cerr << errornum << endl;
}
...
public:
		...
		// класс ostream имел доступ к полям privat у класса stack
	{
		...
		{
			throw stackexception(stackisempty);
		}
		else
		{
...
//то он будет работать не со своей памятью(((
	...
	void push(T DATA)						//запихнуть ячейку на последнее место
	{
		...
		else if (Nownum == Maxnum)throw stackexception(stackisfull);
		else
		{
			newNode->prev = ptrend;
			ptrend->next = newNode;
	...
	{
		if (ptrend == nullptr) throw stackexception(stackisempty);
		else
		{
			ptrend = ptrend->prev;
	...
	T operator [](int index) const
	{
		if (index <=  0 ||index > Maxnum)
		{
			...
			throw stackexception(stackisfull);
		}
		else 
		{
			Node *NOWNUM = ptrbeg;
...
{
	stack <int> Stack(20);
	try {																//исключение при выводе стека 
		cout << Stack;
	}
	catch (stackexception& s)
	{
		cerr << "problem was found out: ";
		s.errorprint();
	}
	try {																//исключение при удалении элемента в пустом стеке 
		Stack.pop();
	}
	catch (stackexception& s)
	{
	...
	for (int i = 0; i != 21; i++)
	{
		try {															//исклюечие при создании узла в заполненном стеке
			Stack.push(i);
		}
		catch (stackexception& s)
		{
